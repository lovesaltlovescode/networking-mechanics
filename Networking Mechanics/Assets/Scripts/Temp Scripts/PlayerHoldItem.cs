...
public enum HeldItem //defaults to int
{
    nothing,
    cucumber,
    egg,
...
public class PlayerHoldItem : NetworkBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Alpha0) && heldItem != HeldItem.nothing)
        {
            CmdChangeHeldItem(HeldItem.nothing);
        }
        if (Input.GetKeyDown(KeyCode.Alpha1) && heldItem != HeldItem.cucumber)
        {
            CmdChangeHeldItem(HeldItem.cucumber);
        }
        if (Input.GetKeyDown(KeyCode.Alpha2) && heldItem != HeldItem.egg)
        {
            CmdChangeHeldItem(HeldItem.egg);
        }
        if (Input.GetKeyDown(KeyCode.Alpha3) && heldItem != HeldItem.chicken)
        {
            CmdChangeHeldItem(HeldItem.chicken);
        }

        //Drop item if player is currently holding something
        if(Input.GetKeyDown(KeyCode.X) && heldItem != HeldItem.nothing)
        {
            CmdDropitem();
        }
    }
